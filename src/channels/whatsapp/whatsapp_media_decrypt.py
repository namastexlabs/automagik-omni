...
class WhatsAppMediaDecryptor:
    ...
    def decrypt_media(self, encrypted_url: str, media_key_b64: str, media_type: int = MEDIA_TYPE_AUDIO) -> Optional[bytes]:
        """
    ...
    def decrypt_and_save_temp(self, encrypted_url: str, media_key_b64: str, media_type: int = MEDIA_TYPE_AUDIO) -> Optional[str]:
        """
    ...
    def _expand_key(self, media_key: bytes, media_type: int) -> Optional[Tuple[bytes, bytes, bytes]]:
        ...
        try:
            ...
            logger.debug(f"Derived keys - IV: {len(iv)} bytes, Cipher: {len(cipher_key)} bytes, MAC: {len(mac_key)} bytes")
    ...
    def _verify_mac(self, encrypted_content: bytes, mac_key: bytes, expected_mac: bytes) -> bool:
        ...
        try:
            ...
            if is_valid:
                ...
                logger.error(f"MAC mismatch - calculated: {calculated_mac_truncated.hex()}, expected: {expected_mac.hex()}")
