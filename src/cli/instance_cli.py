...
def show_instance(name: str):
    ...
    try:
        ...
        console.print(f"Evolution Key: {'*' * len(instance.evolution_key) if instance.evolution_key else '[dim]Not set[/dim]'}")
        ...
        console.print(f"Agent API Key: {'*' * len(instance.agent_api_key) if instance.agent_api_key else '[dim]Not set[/dim]'}")
...
def bootstrap_from_env(
    ...
    make_default: bool = typer.Option(True, help="Make this the default instance")
):
    """Create an instance from current environment variables."""
    create_tables()
    db = get_db()
    try:
        instance = create_instance_from_env(db, name, make_default)
        console.print(f"[green]Instance '{instance.name}' created from environment variables[/green]")
        if instance.is_default:
            console.print(f"[green]Instance '{instance.name}' set as default[/green]")
    finally:
        db.close()
