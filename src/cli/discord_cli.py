...
@app.command("start")
def start_bot(
    instance_name: str = typer.Argument(..., help="Name of the Discord instance to start")
):
    ...
    try:
        ...
        if not discord_service.get_service_status()['service_running']:
            ...
            if not discord_service.start():
                ...
                track_command("discord_start", success=False, error="Service start failed", duration_ms=(time.time() - start_time) * 1000)
                raise typer.Exit(1)
        ...
        if success:
            ...
            track_command("discord_start", success=True, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
        else:
            console.print(f"[red]❌ Failed to start Discord bot '{instance_name}'[/red]")
            track_command("discord_start", success=False, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
            raise typer.Exit(1)
...
@app.command("stop")
def stop_bot(
    instance_name: str = typer.Argument(..., help="Name of the Discord instance to stop")
):
    ...
    try:
        ...
        if success:
            ...
            track_command("discord_stop", success=True, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
        else:
            console.print(f"[red]❌ Failed to stop Discord bot '{instance_name}' (may not be running)[/red]")
            track_command("discord_stop", success=False, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
            raise typer.Exit(1)
...
@app.command("restart")
def restart_bot(
    instance_name: str = typer.Argument(..., help="Name of the Discord instance to restart")
):
    ...
    try:
        ...
        if not discord_service.get_service_status()['service_running']:
            ...
            if not discord_service.start():
                console.print("[red]❌ Failed to start Discord service[/red]")
                track_command("discord_restart", success=False, error="Service start failed", duration_ms=(time.time() - start_time) * 1000)
                raise typer.Exit(1)
        ...
        if success:
            ...
            track_command("discord_restart", success=True, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
        else:
            console.print(f"[red]❌ Failed to restart Discord bot '{instance_name}'[/red]")
            track_command("discord_restart", success=False, instance_name=instance_name, duration_ms=(time.time() - start_time) * 1000)
            raise typer.Exit(1)
...
def list_instances():
    ...
    try:
        ...
        console.print(table)
        track_command("discord_list", success=True, instance_count=len(instances), duration_ms=(time.time() - start_time) * 1000)
...
def _display_bot_status(instance_name: str, bot_status: dict, compact: bool = False):
    ...
    if compact:
        ...
        console.print(f"{status_emoji} {instance_name} - {bot_status.get('guild_count', 0)} guilds, {bot_status.get('user_count', 0)} users")
        return
