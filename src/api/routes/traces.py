...
async def get_trace_analytics(
    ...
    try:
        ...
        avg_processing_time = sum(t.total_processing_time_ms for t in completed_traces) / len(completed_traces) if completed_traces else None
        
        agent_traces = [t for t in traces if t.agent_processing_time_ms is not None]
        avg_agent_time = sum(t.agent_processing_time_ms for t in agent_traces) / len(agent_traces) if agent_traces else None
...
async def cleanup_old_traces(
    ...
    try:
        if dry_run:
            ...
            count = db.query(MessageTrace).filter(
