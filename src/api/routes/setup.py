...
class SetupStatusResponse(BaseModel):
    ...
    requires_setup: bool = Field(..., description="Whether the application requires initial setup")
...
class DatabaseConfigRequest(BaseModel):
    ...
    replication_databases: str | None = Field(
        None, description="Comma-separated database patterns to replicate (empty = all)"
    )
...
async def get_setup_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return SetupStatusResponse(requires_setup=not setup_complete)
...
async def initialize_setup(config: DatabaseConfigRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        def _set_or_update(
            key: str, value: str, value_type: SettingValueType, category: str, description: str, is_secret: bool = False
        ):
        ...
        return {"success": True, "message": "PostgreSQL configuration saved (embedded pgserve)"}
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Setup initialization failed: {str(e)}"
        )
    ...
    try:
        ...
        if setup_setting:
            ...
            if is_already_complete:
                ...
                return SetupCompleteResponse(success=True, message="Setup was already marked as complete")
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to complete setup: {str(e)}"
        )
    ...
    try:
        ...
        if not api_key_setting:
            ...
            return GenerateApiKeyResponse(
                api_key=key_value, message="API key generated successfully. Save this key securely!"
            )

        return GenerateApiKeyResponse(
            api_key=api_key_setting.value or "", message="Your API key is ready. Save this key securely!"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retrieve API key: {str(e)}"
        )
...
async def get_channels_status(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return ChannelStatusResponse(whatsapp=whatsapp_status, discord=discord_status)
        ...
        return ChannelStatusResponse(
            whatsapp=ChannelStatus(
                enabled=False, configured=False, status="error", message=f"Failed to check status: {str(e)}"
            ),
            discord=ChannelStatus(
                enabled=False, configured=False, status="error", message=f"Failed to check status: {str(e)}"
            ),
...
async def _check_whatsapp_status(db: Session) -> ChannelStatus:
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                response = await client.get(f"{evolution_url}/instance/fetchInstances", headers={"apikey": api_key})
                if response.status_code == 200:
                    return ChannelStatus(
                        enabled=True, configured=True, status="ready", message="WhatsApp Web API connected"
                    )
        ...
        return ChannelStatus(enabled=True, configured=False, status="error", message=str(e))
...
async def _check_discord_status(db: Session) -> ChannelStatus:
    ...
    try:
        ...
        discord_instances = db.query(InstanceConfig).filter(InstanceConfig.channel_type == "discord").count()
        ...
        return ChannelStatus(
            enabled=is_enabled, configured=False, status="needs_config", message="No Discord instances configured"
        )

    except Exception as e:
        logger.warning(f"Discord status check failed: {e}")
        return ChannelStatus(enabled=False, configured=False, status="error", message=str(e))


@router.post("/channels/configure", response_model=ChannelConfigResponse)
async def configure_channels(config: ChannelConfigRequest, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        if config.whatsapp_enabled:
            ...
            if config.whatsapp_instance_name:
                ...
                existing_instance = db.query(InstanceConfig).filter(InstanceConfig.name == instance_name).first()
                    ...
                    try:
                        ...
                        if evo_key:
                            ...
                            if evo_instances:
                                ...
                                logger.info(
                                    f"Found existing Evolution instance: {instance_name} (status: {evo_instance.status})"
                                )
                                ...
                                if evo_instance.status == "open":
                                    ...
                                    whatsapp_instance_created = instance_name
                                    logger.info(
                                        f"WhatsApp instance {instance_name} already connected in Evolution - skipping QR"
                                    )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Channel configuration failed: {str(e)}"
        )
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response = await client.get(
                "https://discord.com/api/v10/users/@me", headers={"Authorization": f"Bot {request.bot_token}"}
            )
            ...
            if response.status_code == 200:
                ...
                return {"valid": False, "message": "Invalid bot token"}
            else:
                return {"valid": False, "message": f"Discord API returned {response.status_code}"}

    except httpx.ConnectError:
        return {"valid": False, "message": "Could not connect to Discord API"}
    except Exception as e:
