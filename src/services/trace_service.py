...
class TraceContext:
    ...
    def log_stage(self, 
        ...
        try:
            ...
            logger.debug(f"Logged {stage} payload for trace {self.trace_id} (compressed: {trace_payload.payload_size_compressed} bytes)")
    ...
    def update_trace_status(self, 
        ...
        try:
            trace = self.db_session.query(MessageTrace).filter(
                MessageTrace.trace_id == self.trace_id
            ).first()
...
class TraceService:
    ...
    @staticmethod
    def create_trace(message_data: Dict[str, Any], 
                    instance_name: str,
                    db_session: Session) -> Optional[TraceContext]:
        """
        ...
        try:
            ...
            has_quoted = 'contextInfo' in data and 'quotedMessage' in data.get('contextInfo', {})
    ...
    def get_trace(trace_id: str, db_session: Session) -> Optional[MessageTrace]:
        """Get a trace by ID."""
        try:
            return db_session.query(MessageTrace).filter(
                MessageTrace.trace_id == trace_id
            ).first()
        except Exception as e:
    ...
    @staticmethod
    def get_traces_by_phone(phone: str, 
                           limit: int = 50,
                           db_session: Session = None) -> List[MessageTrace]:
        ...
        try:
            return db_session.query(MessageTrace).filter(
                MessageTrace.sender_phone == phone
            ).order_by(MessageTrace.received_at.desc()).limit(limit).all()
        except Exception as e:
    ...
    def get_trace_payloads(trace_id: str, db_session: Session) -> List[TracePayload]:
        ...
        try:
            return db_session.query(TracePayload).filter(
                TracePayload.trace_id == trace_id
            ).order_by(TracePayload.timestamp.asc()).all()
        except Exception as e:
...
@contextmanager
def get_trace_context(message_data: Dict[str, Any], 
                     instance_name: str) -> Optional[TraceContext]:
    """
