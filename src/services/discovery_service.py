...
class DiscoveryService:
            ...
            try:
                ...
                for evo_instance in evolution_instances:
                    ...
                    existing_instance = db.query(InstanceConfig).filter(
                        InstanceConfig.name == evo_instance.instanceName
                    ).first()
    ...
    async def _create_instance_from_evolution(self, evo_instance: EvolutionInstance, db: Session, evolution_url: str, evolution_key: str) -> Optional[InstanceConfig]:
        """
    ...
    async def sync_instance_status(self, instance_name: str, db: Session) -> Optional[Dict[str, Any]]:
        ...
        db_instance = db.query(InstanceConfig).filter(
