...
class CorsConfig(BaseModel):
    ...
    allowed_origins: List[str] = Field(
        default_factory=lambda: [
            origin.strip() 
            for origin in os.getenv("AUTOMAGIK_OMNI_CORS_ORIGINS", "http://localhost:3000,http://localhost:8888").split(",")
            if origin.strip()
        ]
    )
    allow_credentials: bool = Field(
        default_factory=lambda: os.getenv("AUTOMAGIK_OMNI_CORS_ALLOW_CREDENTIALS", "true").lower() == "true"
    )
    allow_methods: List[str] = Field(
        default_factory=lambda: [
            method.strip() 
            for method in os.getenv("AUTOMAGIK_OMNI_CORS_ALLOW_METHODS", "*").split(",")
            if method.strip()
        ] or ["*"]
    )
    allow_headers: List[str] = Field(
        default_factory=lambda: [
            header.strip() 
            for header in os.getenv("AUTOMAGIK_OMNI_CORS_ALLOW_HEADERS", "*").split(",")
            if header.strip()
        ] or ["*"]
    )
