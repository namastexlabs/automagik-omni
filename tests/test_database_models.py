...
from src.db.bootstrap import ensure_default_instance, create_instance_from_env
...
class TestBootstrapFunctions:
    ...
    def mock_config(self):
        """Mock config for bootstrap tests."""
        with patch('src.db.bootstrap.config') as mock_config:
            mock_config.whatsapp.instance = "test-instance"
            mock_config.whatsapp.session_id_prefix = "test-"
            mock_config.agent_api.url = "http://test-agent-api"
            mock_config.agent_api.api_key = "test-key"
            mock_config.agent_api.default_agent_name = "test-agent"
            mock_config.agent_api.timeout = 60
            yield mock_config
    
    def test_ensure_default_instance_creates_new(self, test_db):
        """Test that ensure_default_instance creates a new instance when none exist."""
        ...
        assert instance.whatsapp_instance == "test-instance"  # From test env
    ...
    def test_create_instance_from_env(self, test_db):
        """Test creating instance from environment variables."""
        instance = create_instance_from_env(test_db, "env_test", make_default=True)
        
        assert instance.name == "env_test"
        assert instance.is_default is True
        assert instance.whatsapp_instance == "test-instance"  # From test env
        assert instance.agent_api_url == "http://test-agent-api"  # From test env
        assert instance.default_agent == "test-agent"  # From test env
    
    def test_create_instance_from_env_duplicate_name(self, test_db, default_instance_config):
        """Test that creating instance with existing name returns existing instance."""
        # Try to create instance with existing name
        instance = create_instance_from_env(test_db, "default", make_default=False)
        
        # Should return the existing instance
        assert instance.id == default_instance_config.id
        assert instance.name == "default"
    
    def test_create_instance_from_env_unsets_other_defaults(self, test_db, default_instance_config):
        """Test that making new instance default unsets other defaults."""
        # Create new instance as default
        new_instance = create_instance_from_env(test_db, "new_default", make_default=True)
        
        # Refresh the original default instance
        ...
        assert new_instance.is_default is True
