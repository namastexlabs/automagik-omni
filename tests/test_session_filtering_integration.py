...
class TestSessionFilteringIntegration:
    ...
    def setup_test_data(self, db: Session):
        ...
        db.query(MessageTrace).filter(
            MessageTrace.instance_name.in_(
                ["session_filter_test_a", "session_filter_test_b"]
            )
        ).delete()
    ...
    def test_filter_by_agent_session_id_functional(self, test_client, test_db):
        ...
        created_traces = (
            test_db.query(MessageTrace)
            .filter(MessageTrace.trace_id.like("test_%"))
            .all()
        )
        print(f"\nDEBUG: Created {len(created_traces)} test traces in DB")
        for trace in created_traces:
            print(
                f"  - {trace.trace_id}: instance={trace.instance_name}, agent_session={trace.agent_session_id}"
            )
        ...
        try:
            ...
            response_all = test_client.get(
                "/api/v1/traces", headers={"x-api-key": "namastex888"}
            )
            ...
            test_traces_only = [
                t for t in traces if t.get("trace_id", "").startswith("test_")
            ]
            ...
            agent_session_traces = [
                ...
                if t.get("agent_session_id") == target_session
            ]
    ...
    def test_filter_by_session_name_functional(self, test_client, test_db):
        ...
        try:
            ...
            if response.status_code != 200:
                print(
                    f"ERROR: Status {response.status_code}, Response: {response.json()}"
                )
            assert response.status_code == 200
            traces = response.json()
            ...
            session_traces = [
                t for t in test_traces_only if t.get("session_name") == target_session
            ]
    ...
    def test_filter_by_has_media_functional(self, test_client, test_db):
        ...
        try:
            ...
            test_media_trace = next(
                (t for t in media_traces if t.get("trace_id") == "test_trace_004"), None
            )
    ...
    def test_combined_session_filters_functional(self, test_client, test_db):
        ...
        try:
            ...
            filtered_traces = [
                t
                for t in test_traces_only
                if t.get("session_name") == target_session
                and t.get("instance_name") == target_instance
            ]
    ...
    def test_session_isolation_functional(self, test_client, test_db):
        ...
        try:
            ...
            test_traces_a = [
                t for t in traces_a if t.get("trace_id", "").startswith("test_")
            ]
            test_traces_b = [
                t for t in traces_b if t.get("trace_id", "").startswith("test_")
            ]
    ...
    def test_phone_alias_parameter_functional(self, test_client, test_db):
        ...
        try:
            ...
            non_test_traces = [
                t for t in all_traces_in_db if not t.trace_id.startswith("test_")
            ]
            if non_test_traces:
                print(f"\nDEBUG: Found {len(non_test_traces)} non-test traces:")
                for trace in non_test_traces[:3]:  # Show first 3
                    print(
                        f"  - {trace.trace_id}: instance={trace.instance_name}, phone={trace.sender_phone}"
                    )
            ...
            test_traces1 = [
                t for t in traces1 if t.get("trace_id", "").startswith("test_")
            ]
            test_traces2 = [
                t for t in traces2 if t.get("trace_id", "").startswith("test_")
            ]
            ...
            print(
                f"Response 2 (sender_phone): {len(traces2)} total traces, {len(test_traces2)} test traces"
            )
            if traces1:
                print(
                    f"All traces from phone filter: {[t['trace_id'] for t in traces1]}"
                )
            if test_traces1:
    ...
    def test_pagination_with_session_filtering_functional(self, test_client, test_db):
        ...
        try:
            ...
            test_traces_only_offset = [
                t for t in traces_offset if t.get("trace_id", "").startswith("test_")
            ]
            session_traces_offset = [
                t
                for t in test_traces_only_offset
                if t.get("session_name") == target_session
            ]
            ...
            if len(session_traces_offset) > 0:
                ...
                assert (
                    session_traces[0]["trace_id"]
                    != session_traces_offset[0]["trace_id"]
                )
