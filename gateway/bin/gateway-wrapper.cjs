#!/usr/bin/env node
/**
 * Gateway wrapper - Finds and spawns bun runtime from node_modules
 *
 * This wrapper enables `npx @automagik/gateway` to work without requiring
...
if (!bunPath) {
  ...
  console.error('');
  console.error('Try reinstalling: npm install @automagik/gateway');
  process.exit(1);
}
